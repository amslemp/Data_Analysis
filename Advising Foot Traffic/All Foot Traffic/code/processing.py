...
def clean_location_data(all_location_df, saved_file_name):
    ...
    all_location_df['DATE'] = pd.to_datetime(all_location_df['DATE'], errors = 'coerce')
    ...
    mon = [month_dict.get(i) for i in list(all_location_df['DATE'].dt.month)]
    
    #Create dictionary for days of the week
    day_of_week = {0: 'Mon', 1:'Tues', 2:'Wed', 3:'Thur', 4:'Fri', 5:'Sat', 6:'Sun'}
    
    #Comprehension for Days
    dow = [day_of_week.get(i) for i in list(all_location_df['DATE'].dt.weekday)]
    
    #Combine new columns with old fin_mashup df
    all_location_df['MONTH'], all_location_df['DAY'] = mon, dow
